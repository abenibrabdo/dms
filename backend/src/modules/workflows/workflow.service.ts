...
import { WorkflowModel, WorkflowStepModel, type WorkflowInstance, type WorkflowStep } from './workflow.model.js';
...
export const toWorkflowInstance = (workflow: WorkflowModel): WorkflowInstance => {
  const steps = (workflow.steps ?? []).map((step) => {
    const plain = step.get({ plain: true }) as WorkflowStep;
    return plain;
  });
...
export const createWorkflow = async (input: CreateWorkflowInput): Promise<WorkflowInstance> => {
  const workflowId = await sequelize.transaction(async (transaction) => {
    ...
    if (input.steps.length > 0) {
      await WorkflowStepModel.bulkCreate(
...
export const listWorkflows = async (): Promise<WorkflowInstance[]> => {
...
export const getWorkflow = async (workflowId: string): Promise<WorkflowInstance> => {
