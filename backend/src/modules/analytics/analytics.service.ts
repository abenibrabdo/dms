...
const reduceSummary = (rows: Array<{ status?: string; count: number | string }>, fallback = 'unknown') => {
...
export const getDashboardSummary = async () => {
  ...
  const documentSummary = reduceSummary(documentStats as Array<{ status?: string; count: number | string }>);
  const workflowSummary = reduceSummary(workflowStats as Array<{ status?: string; count: number | string }>);
  const notificationSummary = reduceSummary(
    notificationStats as Array<{ status?: string; count: number | string }>,
  );
...
export const getActivityTrend = async (days = 14) => {
  ...
  return {
    ...
    documents: documentTrend as Array<Record<string, string>>,
    workflows: workflowTrend as Array<Record<string, string>>,
    notifications: notificationTrend as Array<Record<string, string>>,
  };
};
