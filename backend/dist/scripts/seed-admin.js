...
const main = async () => {
    ...
    if (existing) {
        logger.info({ email: env.adminSeed.email }, 'Admin user already exists. Skipping seed operation.');
        return;
    }
...
void main()
    ...
    .finally(async () => {
    await disconnectDatabase().catch((error) => logger.error({ err: error }, 'Failed to close database connection'));
});
