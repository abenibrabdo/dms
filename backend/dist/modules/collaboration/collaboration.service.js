...
export const addComment = async (input) => {
    ...
    logger.info({ documentId: document.id, commentId: comment.id, authorId: input.authorId }, 'Comment posted on document');
...
export const listComments = async (documentId) => {
...
export const lockDocument = async (input, force = false) => {
    const document = await DocumentModel.findByPk(input.documentId);
    if (!document) {
...
export const unlockDocument = async (input) => {
    const document = await DocumentModel.findByPk(input.documentId);
    if (!document) {
