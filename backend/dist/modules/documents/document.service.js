...
import { DocumentModel, DocumentVersionModel, } from './document.model.js';
...
export const toManagedDocument = (document) => {
    const versionModels = document.versions ?? document.get('versions') ?? [];
...
export const createDocument = async (input, createdBy) => {
...
export const updateDocumentMetadata = async (id, input, updatedBy) => {
    const document = await DocumentModel.findByPk(id);
    if (!document) {
        throw new NotFoundError('Document not found');
    }
...
export const addDocumentVersion = async (input) => {
...
export const getDocumentVersionMetadata = async (documentId, versionNumber) => {
    ...
    const versions = document.get('versions') ?? [];
    const resolvedVersion = versionNumber !== undefined
